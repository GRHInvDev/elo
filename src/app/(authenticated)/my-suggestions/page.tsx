...
type SuggestionStatus = "NEW" | "IN_REVIEW" | "APPROVED" | "IN_PROGRESS" | "DONE" | "NOT_IMPLEMENTED"
...
export default function MySuggestionsPage() {
  ...
  const filteredSuggestions = useMemo(() => {
    return userSuggestions.filter((suggestion: any) => {
      const matchesSearch = searchTerm === "" ||
        suggestion.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
        suggestion.problem?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        suggestion.ideaNumber.toString().includes(searchTerm)
  ...
  const stats = useMemo(() => {
    ...
    const approved = userSuggestions.filter((s: any) => s.status === "APPROVED" || s.status === "IN_PROGRESS" || s.status === "DONE").length
    const inReview = userSuggestions.filter((s: any) => s.status === "IN_REVIEW").length
    const completed = userSuggestions.filter((s: any) => s.status === "DONE").length
          ...
          {filteredSuggestions.length === 0 ? (
            ...
            <Accordion type="single" collapsible className="w-full">
              {filteredSuggestions.map((suggestion: any) => {
                const statusConfig = STATUS_CONFIG[suggestion.status as SuggestionStatus]
                const StatusIcon = statusConfig.icon

                return (
                  <AccordionItem key={suggestion.id} value={suggestion.id} className="border-b border-border">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-semibold text-foreground dark:text-foreground">
                                #{suggestion.ideaNumber}
                              </span>
                              <Badge className={statusConfig.color}>
                                {STATUS_MAPPING[suggestion.status as SuggestionStatus]}
                              </Badge>
                            </div>
                            <p className="text-sm text-muted-foreground dark:text-muted-foreground truncate">
                              {suggestion.problem || "Sem problema definido"}
                            </p>
                          </div>
                        </div>
                    ...
                    <AccordionContent className="px-6 pb-4">
                      <div className="space-y-4">
                        ...
                        <div className="p-4 rounded-lg bg-muted/30 dark:bg-muted/30 border">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="font-medium text-foreground dark:text-foreground">
                              Status: {STATUS_MAPPING[suggestion.status as SuggestionStatus]}
                            </span>
                          </div>
                              ...
                              <div className="mt-1">
                                <Badge variant="outline">
                                  {getContributionTypeLabel(
                                    suggestion.contribution?.type || "",
                                    suggestion.contribution?.other
                                  )}
                                </Badge>
                              </div>
                            </div>
                        ...
                        {suggestion.status === "NOT_IMPLEMENTED" && suggestion.rejectionReason && (
