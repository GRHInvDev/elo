...
export default async function NewsDisplay() {
  ...
  return (
    <div className="flex flex-col max-w-[100dvw] overflow-hidden gap-y-16">
      {
        posts.map((p, i)=>(
          <div key={i} className="border-t-2 pt-4 group flex even:flex-row-reverse even:text-end even:translate-x-32 odd:-translate-x-32 md:even:translate-x-0 md:odd:translate-x-0 relative gap-4">
            <div>
            ...
            <div className="min-w-[60dvw] space-y-3 overflow-hidden">
              <h1 className="text-md font-bold md:text-2xl max-w-xl overflow-ellipsis text-wrap group-even:place-self-end">{p.title}</h1>
