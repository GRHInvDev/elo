...
export default async function NewsDisplay() {
  ...
  return (
    <div className="flex flex-col max-w-[100dvw] overflow-hidden gap-y-16">
      {
        posts.map((p, i)=>(
          <div key={i} className="border-t-2 pt-4 group flex relative gap-4">
            <div>
            ...
            <div className="min-w-[60dvw] space-y-3 overflow-hidden">
              <h1 className="text-md font-bold md:text-2xl max-w-xl overflow-ellipsis text-wrap">{p.title}</h1>
              ...
              <p className="mb-4 text-xs max-w-xl md:text-sm md:line-clamp-2 line-clamp-4">{p.content}</p>
